...
export const addUserToRoom = async function (
	...
	if (!skipSystemMessage) {
		if (inviter) {
			...
			if (room.teamMain) {
				await Message.saveSystemMessage('added-user-to-team', rid, userToBeAdded.name? userToBeAdded.name : '', userToBeAdded, extraData);
			} else {
				await Message.saveSystemMessage('au', rid, userToBeAdded.name? userToBeAdded.name : '', userToBeAdded, extraData);
			}
		} else if (room.prid) {
			await Message.saveSystemMessage('ut', rid, userToBeAdded.name? userToBeAdded.name : '', userToBeAdded, { ts: now });
		} else if (room.teamMain) {
			await Message.saveSystemMessage('ujt', rid, userToBeAdded.name? userToBeAdded.name : '', userToBeAdded, { ts: now });
		} else {
			await Message.saveSystemMessage('uj', rid, userToBeAdded.name? userToBeAdded.name : '', userToBeAdded, { ts: now });
		}
	}
