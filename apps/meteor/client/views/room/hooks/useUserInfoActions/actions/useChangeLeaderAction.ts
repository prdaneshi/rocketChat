...
export const useChangeLeaderAction = (user: Pick<IUser, '_id' | 'username' | 'name' >, rid: IRoom['_id']): UserInfoAction | undefined => {
	...
	const changeLeader = useEndpointAction('POST', `${endpointPrefix}.${changeLeaderEndpoint}`, {
		successMessage: t(changeLeaderMessage, { name: user.name, role: 'leader' }),
	});
